...
function Inicio() {
    ...
    const cargarProductos = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${API_URL}ropa-caballero.php`);
            ...
            <MainContainer>
                <MainScrollView showsVerticalScrollIndicator={false}>
                    <Container>
                        ...
                        <SectionContainer>
                            <SectionHeader>
                                <Titulo>Ropa de Caballero disponible</Titulo>
                                <ViewModeContainer>
                                    <ViewModeButton 
...
const ProductoCategoria = styled(Text)`
    font-size: 12px;
    color: #FFF;
    background-color: #FFB3B3; /* Color pastel */
    padding: 2px 8px;
    border-radius: 10px;
...
const ProductoAtributo = styled(View)`
    ...
    margin-right: 10px;
    margin-bottom: 5px;
    background-color: #FFEBEB; /* Color pastel */
    padding: 3px 8px;
    border-radius: 10px;
`;
...
// Contenedor con fondo pastel
const GradientBackground = styled(LinearGradient).attrs({
  colors: [
    'rgba(255, 204, 204, 0.8)',  // Color pastel rojo
    'rgba(255, 223, 223, 0.85)', // Color pastel más claro
    'rgba(255, 204, 204, 0.9)',  // Color pastel rojo
    'rgba(255, 182, 182, 0.9)'   // Color pastel más oscuro
  ],
  start: { x: 0, y: 0 },
  end: { x: 1, y: 1 },
})`
...
// Capa de blur encima del fondo pastel
const BlurredOverlay = styled(BlurView).attrs({
  intensity: 40,
  tint: 'light', // Mejor para fondos claros
})`
  flex: 1;
  position: absolute;
  width: 100%;
...
const HeaderSection = styled(View)`
    padding: 20px;
    padding-top: 40px;
    background-color: rgba(255, 87, 34, 0.8); /* Rojo intenso */
`;
...
const MenuButton = styled(TouchableOpacity)`
    padding: 8px;
    border-radius: 8px;
    background-color: rgba(255, 87, 34, 0.5); /* Rojo suave */
`;
...
const Titulo = styled(Text)`
    ...
    font-weight: bold;
    margin-bottom: 15px;
    color: rgba(255, 87, 34, 0.8); /* Rojo intenso */
    text-align: center;
`;
...
const ViewModeButton = styled(TouchableOpacity)<{ active: boolean }>`
    ...
    padding: 10px 16px;
    border-radius: 18px;
    background-color: ${props => props.active ? 'rgba(255, 87, 34, 0.8)' : 'transparent'}; /* Rojo suave */
    margin: 0 2px;
`;
...
const ProductoContainer = styled(LinearGradient).attrs({
  colors: ['rgba(255, 204, 204, 0.85)', 'rgba(255, 87, 34, 0.85)'], // Rojo pastel y rojo intenso
  start: { x: 0, y: 0 },
  end: { x: 1, y: 1 },
})`
  flex-direction: row;
